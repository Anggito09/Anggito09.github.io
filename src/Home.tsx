...
export default function Home() {
  ...
  useEffect(() => {
    const sections = ["top", "kamera", "banding", "cetak-foto", "cara", "cerita", "faq"]
  ...
  return <main>
    <div className="scroll-progress" aria-hidden="true"><span style={{transform:`scaleX(${scrollProgress})`}}></span></div><nav className="nav-shell"><div className="nav wrap"><a className="logo-link" href="#top" aria-label="KameraIn"><img src={`${import.meta.env.BASE_URL}kamerain-logo-v2.webp`} alt="Logo KameraIn"/></a><div className="navlinks"><a className={activeSection==="kamera"?"active":""} href="#kamera">Kamera</a><a className={activeSection==="banding"?"active":""} href="#banding">Bandingkan</a><a className={activeSection==="cetak-foto"?"active":""} href="#cetak-foto">Cetak Foto</a><a className={activeSection==="cara"?"active":""} href="#cara">Cara Sewa</a><a className={activeSection==="cerita"?"active":""} href="#cerita">Galeri</a><a className={activeSection==="faq"?"active":""} href="#faq">FAQ</a></div><a className="btn small" href="#booking">Booking sekarang v</a></div></nav>
    ...
    <section id="cetak-foto" className="print-service-section" data-reveal><div className="wrap print-service"><div className="print-service-copy"><span className="eyebrow coral">TANPA SEWA KAMERA</span><h2>Jasa Cetak Foto Saja</h2><p>Kirim foto digital favoritmu dan KameraIn akan mencetaknya menjadi foto Instax Mini yang siap disimpan atau diberikan kepada orang tersayang.</p><div className="print-service-points"><span>✓ Tidak perlu menyewa kamera</span><span>✓ Harga dihitung per foto</span><span>✓ File dikirim melalui WhatsApp</span></div></div><div className="print-service-card"><small>MULAI DARI</small><strong>Rp18.000</strong><span>per foto</span><p>Jumlah foto dan waktu pengerjaan akan dikonfirmasi oleh admin KameraIn.</p><a className="btn wide" href="https://wa.me/6283193266639?text=Halo%20KameraIn!%20Saya%20ingin%20pesan%20jasa%20cetak%20foto%20saja%20dengan%20harga%20Rp18.000%20per%20foto.%20Mohon%20info%20cara%20kirim%20fotonya%20ya." target="_blank" rel="noreferrer">Pesan Cetak Foto via WhatsApp</a></div></div></section>
