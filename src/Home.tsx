...
const officialCameraGuides = [
  {
    ...
    specifications: [
      ["Film", "FUJIFILM instaxâ¢ mini"], ["Ukuran foto", "62 Ã 46 mm"],
      ["Jarak pemotretan", "Mulai 0,3 m; Close-Up 0,3â0,5 m"], ["Shutter", "1/2â1/250 detik; slow sync"],
      ["Exposure", "Otomatis, Lv 5.0â14.5 (ISO 800)"], ["Berat", "306 g tanpa baterai, strap, dan film"],
    ],
    design: "Bodi membulat dengan lens ring untuk menyalakan kamera dan berpindah ke Close-Up Mode.",
    movies: "Video promosi âFill your world with joyâ dan tutorial penggunaan resmi.",
    films: "Menggunakan film instaxâ¢ mini; pilihan bingkai dapat disesuaikan dengan paket yang tersedia.",
    accessories: "Aksesori seri instaxâ¢ mini, termasuk case dan strap yang kompatibel.",
  },
  {
    slug: "mini13",
    ...
    specifications: [
      ["Film", "FUJIFILM instaxâ¢ mini"], ["Ukuran foto", "62 Ã 46 mm"],
      ["Jarak pemotretan", "Mulai 0,3 m; Close-Up 0,3â0,5 m"], ["Self-timer", "Sekitar 2 atau 10 detik"],
      ["Exposure", "Otomatis, Lv 5.0â14.5 (ISO 800)"], ["Berat", "327 g tanpa baterai, film, dan strap"],
    ],
    ...
    films: "Kompatibel dengan film instaxâ¢ mini berukuran gambar 62 Ã 46 mm.",
    accessories: "Lihat pilihan aksesori resmi yang dirancang untuk seri instaxâ¢ mini 13.",
  },
  {
    slug: "mini41",
    ...
    specifications: [
      ["Film", "FUJIFILM instaxâ¢ mini"], ["Ukuran foto", "62 Ã 46 mm"],
      ["Jarak pemotretan", "Mulai 0,3 m; Close-Up 0,3â0,5 m"], ["Shutter", "1/2â1/250 detik; slow sync"],
      ["Flash", "Otomatis; jangkauan 0,3â2,2 m"], ["Berat", "345 g tanpa baterai, film, dan strap"],
    ],
    ...
    films: "Menggunakan film instaxâ¢ mini dengan waktu pengembangan sekitar 90 detik, bergantung suhu sekitar.",
  ...
  },
  {
    ...
    specifications: [
      ["Sensor", "CMOS 1/5 inci; 2560 Ã 1920"], ["Efek", "10 lens Ã 10 film = 100 kombinasi"],
      ["Layar", "LCD TFT warna 3 inci"], ["Jarak pemotretan", "10 cm hingga tak terhingga"],
      ["Konektivitas", "Bluetooth 4.2 BLE"], ["Cetak", "Sekitar 16 detik; Â±100 cetakan/isi penuh"],
    ],
    ...
    films: "Mencetak pada film instaxâ¢ mini dan mendukung pencetakan gambar terpilih dari memori kamera.",
    accessories: "Mendukung microSD/microSDHC serta aksesori yang sesuai untuk kamera hybrid mini Evo.",
  },
] as const;

const officialGuideCards = [
  { key: "specifications", icon: "â", title: "Specifications", note: "Spesifikasi teknis lengkap" },
  { key: "design", icon: "â¦", title: "Product Design", note: "Bagian kamera dan desain" },
  { key: "movies", icon: "â¶", title: "Movies", note: "Video promosi dan tutorial" },
  { key: "films", icon: "â£", title: "Films", note: "Film yang kompatibel" },
  { key: "accessories", icon: "ï¼", title: "Accessories", note: "Aksesori pendukung" },
] as const;
const filmChoices = [
...
export default function Home() {
  ...
  return <main>
    <div className="scroll-progress" aria-hidden="true"><span style={{transform:`scaleX(${scrollProgress})`}}></span></div><nav className="nav-shell"><div className="nav wrap"><a className="logo-link" href="#top" aria-label="KameraIn"><img src={`${import.meta.env.BASE_URL}kamerain-logo-v2.webp`} alt="Logo KameraIn"/></a><div className="navlinks"><a className={activeSection==="kamera"?"active":""} href="#kamera">Kamera</a><a className={activeSection==="banding"?"active":""} href="#banding">Bandingkan</a><a className={activeSection==="cara"?"active":""} href="#cara">Cara Sewa</a><a className={activeSection==="cerita"?"active":""} href="#cerita">Galeri</a><a className={activeSection==="faq"?"active":""} href="#faq">FAQ</a></div><a className="btn small" href="#booking">Booking sekarang â</a></div></nav>
    <section id="top" className="hero wrap"><div className="hero-copy"><span className="eyebrow">ð¸ SEWA INSTAX â¢ JAKARTA & SEKITARNYA</span><h1>Tangkap momennya.<br/><em>Cetak ceritanya.</em></h1><p>Sewa kamera Instax untuk wisuda, ulang tahun, pernikahan, atau jalan-jalan. Praktis, terawat, dan siap bikin momenmu lebih berkesan.</p><div className="hero-actions"><a className="btn" href="#kamera">Pilih kamera â</a><a className="text-link" href="#booking">Konfirmasi booking</a></div><div className="trust"><span>â 4 pilihan kamera</span><span>â Tutorial singkat</span><span>â Durasi fleksibel</span></div><div className="proof-row"><span><b>01</b> Unit diperiksa sebelum disewakan</span><span><b>02</b> Pilihan paket transparan</span><span><b>03</b> Booking tercatat & dikonfirmasi via WhatsApp</span></div></div><div className="hero-art"><div className="blob"></div><div className="hero-camera-grid">{cameras.map((item,index)=><button key={item.name} className={`hero-camera camera-${index+1}`} onClick={()=>{setCamera(index);document.getElementById("kamera")?.scrollIntoView()}} aria-label={`Pilih ${item.name}`}><img src={item.image} alt={item.name}/><span>{item.name.replace("Instax ","")}</span></button>)}</div><span className="spark s1">â¦</span><span className="spark s2">â¿</span></div></section>
    <section className="occasion-strip" data-reveal><div className="wrap occasions"><div><span className="eyebrow coral">SATU KAMERA, BANYAK CERITA</span><h2>Instax cocok untuk momen apa saja?</h2><p>Dari acara intim sampai hari besar, hasil cetaknya bisa langsung dibagikan dan dibawa pulang sebagai kenang-kenangan.</p></div><div className="occasion-list"><span>ð Wisuda</span><span>ð Pernikahan</span><span>ð Ulang tahun</span><span>ð Gathering</span><span>âï¸ Liburan</span><span>ð Bridal shower</span></div></div></section>
    <section id="kamera" className="section wrap" data-reveal><div className="section-head"><div><span className="eyebrow coral">4 KAMERA TERSEDIA</span><h2>Kamera untuk setiap cerita</h2></div><p>Klik bagian mana saja pada kartu kamera. Detail dan harga paket di bawah akan langsung ikut berubah.</p></div><div className="camera-grid" role="radiogroup" aria-label="Pilih kamera">{cameras.map((item,index)=><article key={item.name} role="radio" aria-checked={camera===index} tabIndex={0} className={`camera-card ${camera===index?"active":""}`} style={{animationDelay:`${index*80}ms`}} onClick={()=>setCamera(index)} onKeyDown={(e)=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();setCamera(index)}}}><span className="tag">{item.tag}</span><img src={item.image} alt={item.name}/><div><small>{item.brand} â¢ {item.type}</small><h3>{item.name}</h3><p className="card-highlight">{item.highlight}</p><div className="mini-prices"><span><small>3 Jam</small><b>{rupiah(item.prices["3 Jam"])}</b></span><span><small>6 Jam</small><b>{rupiah(item.prices["6 Jam"])}</b></span><span><small>1 Hari</small><b>{rupiah(item.prices["1 Hari"])}</b></span></div></div><span className="choose">{camera===index?"â Kamera terpilih":"Klik untuk memilih"}</span></article>)}</div><div key={camera} className="camera-detail" aria-live="polite"><div className="detail-photo"><span>{cameras[camera].tag}</span><img src={cameras[camera].image} alt={cameras[camera].name}/></div><div className="detail-copy"><span className="eyebrow coral">KENALI KAMERANYA</span><small>{cameras[camera].brand} â¢ {cameras[camera].type} â¢ {cameras[camera].color}</small><h2>{cameras[camera].name}</h2><p>{cameras[camera].highlight}</p><div className="best-for"><b>Paling cocok untuk</b><span>{cameras[camera].bestFor}</span></div><div className="spec-grid">{cameras[camera].specs.map(spec=><span key={spec}>â {spec}</span>)}</div><div className="detail-actions"><button type="button" className="btn" onClick={()=>document.querySelector(".booking-band")?.scrollIntoView()}>Pilih durasi & paket â</button><a className="text-link" href="#booking">Booking kamera ini</a></div></div></div><p className="spec-source">Spesifikasi produk diringkas dari informasi resmi FUJIFILM instaxâ¢. Film tersedia terpisah sesuai paket yang dipilih.</p></section>
    <section className="official-guide-section" data-reveal><div className="official-guide"><div className="official-guide-visual"><span className="eyebrow coral">PANDUAN KAMERA</span><img src={cameras[camera].image} alt={cameras[camera].name}/><div><small>FUJIFILM instaxâ¢</small><h2>{cameras[camera].name}</h2><p>{officialCameraGuides[camera].intro}</p></div></div><div className="official-guide-content"><div className="official-guide-heading"><div><span className="eyebrow coral">DETAIL RESMI PRODUK</span><h3>Kenali kamera sebelum menyewa</h3></div><span className="source-pill">Sumber: FUJIFILM Indonesia</span></div><div className="official-spec-list">{officialCameraGuides[camera].specifications.map(([label,value])=><div key={label}><span>{label}</span><b>{value}</b></div>)}</div><div className="official-link-grid">{officialGuideCards.map(item=><a key={item.key} href={`https://www.fujifilm.com/id/en/consumer/instax/cameras/${officialCameraGuides[camera].slug}/${item.key}`} target="_blank" rel="noreferrer"><i>{item.icon}</i><span><b>{item.title}</b><small>{item.note}</small></span><strong>â</strong></a>)}</div><div className="official-guide-notes"><article><b>Product Design</b><p>{officialCameraGuides[camera].design}</p></article><article><b>Movies</b><p>{officialCameraGuides[camera].movies}</p></article><article><b>Films</b><p>{officialCameraGuides[camera].films}</p></article><article><b>Accessories</b><p>{officialCameraGuides[camera].accessories}</p></article></div><p className="official-disclaimer">Ringkasan dibuat berdasarkan informasi resmi FUJIFILM. Tautan membuka halaman produk FUJIFILM Indonesia pada tab baru; ketersediaan film dan aksesori dalam paket sewa mengikuti stok KameraIn.</p></div></div></section>
    <section className="film-showcase" data-reveal><div className="wrap"><div className="section-head"><div><span className="eyebrow coral">PILIH GAYA CETAKMU</span><h2>Film mini untuk setiap cerita</h2></div><p>Contoh desain resmi film FUJIFILM instaxâ¢ mini. Pilihan motif mengikuti stok saat tanggal penyewaan.</p></div><div className="film-scroll" aria-label="Pilihan desain film Instax Mini">{filmChoices.map((film,index)=><article className="film-card" key={film.name} style={{animationDelay:`${index*55}ms`}}><div className="film-image"><img src={film.image} alt={`Kemasan resmi film Instax Mini ${film.name}`} loading="lazy"/><span>{index===2?"NEW":"10 SHEETS"}</span></div><h3>{film.name}</h3><p>{film.note}</p></article>)}</div><div className="film-foot"><span>â¹ï¸ Gambar produk: FUJIFILM Indonesia</span><a href="https://www.fujifilm.com/id/en/consumer/instax/cameras/mini13/films" target="_blank" rel="noreferrer">Lihat katalog film resmi â</a></div><p className="film-stock-note">Katalog ini adalah referensi pilihan desain, bukan informasi stok real-time. Motif yang tersedia akan dikonfirmasi admin ketika booking.</p></div></section>
    ...
    <section id="booking" className="booking-band" data-reveal><div className="wrap booking"><div className="booking-title"><span className="eyebrow">BUAT PAKETMU</span><h2>Sewa sesuai kebutuhanmu</h2><p>Pilih kamera, durasi sewa, dan paket film Instax.</p><div className="film-price"><span>Refill putih 10 lembar <b>Rp140.000</b></span><span>Refill motif 10 lembar <b>Rp150.000</b></span></div></div><div className="builder"><div className="builder-top"><div><small>KAMERA PILIHAN</small><b>{cameras[camera].name}</b></div><img src={cameras[camera].image} alt={cameras[camera].name}/></div><label>1. Pilih kamera</label><div className="camera-tabs">{cameras.map((item,index)=><button type="button" key={item.name} onClick={()=>setCamera(index)} className={camera===index?"selected":""}><img src={item.image} alt=""/><span>{item.name.replace("Instax ","")}</span></button>)}</div><label>2. Pilih durasi</label><div className="segmented">{durations.map(d=><button type="button" key={d} onClick={()=>setDuration(d)} className={duration===d?"selected":""}><b>{d}</b><small>{rupiah(cameras[camera].prices[d])}</small></button>)}</div><label>3. Pilih kamera saja atau bundling</label><div className="package-list">{packages.map(p=><button type="button" key={p.id} onClick={()=>setPackageId(p.id)} className={packageId===p.id?"selected":""}><span className="radio"></span><span><b>{p.name}</b><small>{p.note}</small></span><strong>{p.extra?`+${rupiah(p.extra)}`:"Termasuk"}</strong></button>)}</div><div key={`${camera}-${duration}-${packageId}`} className="total"><span><small>Total pilihan</small><b>{cameras[camera].name} â¢ {duration} â¢ {selectedPackage.name}</b></span><strong>{rupiah(total)}</strong></div><button type="button" className="btn wide" onClick={()=>{setBookingState("idle");setBookingOpen(true)}}>ð¬ Konfirmasi harga & booking</button><small className="price-note">Booking dicatat dengan status Menunggu, lalu ketersediaan dikonfirmasi melalui WhatsApp.</small></div></div></section>
    <section id="cara" className="section wrap" data-reveal><div className="center"><span className="eyebrow coral">GAMPANG BANGET</span><h2>3 langkah, langsung jepret</h2></div><div className="steps"><article><span>01</span><div>ð·</div><h3>Pilih kamera</h3><p>Tentukan kamera, durasi, dan kebutuhan paper.</p></article><article><span>02</span><div>ð¬</div><h3>Konfirmasi jadwal</h3><p>Chat WhatsApp untuk cek unit dan lokasi serah terima.</p></article><article><span>03</span><div>â¨</div><h3>Ambil & abadikan</h3><p>Terima unit bersih beserta tutorial singkatnya.</p></article></div></section>
    <section id="cerita" className="section gallery-section" data-reveal><div className="wrap"><div className="section-head"><div><span className="eyebrow coral">FOTO & VIDEO KAMERAIN</span><h2>Momen kecil, kenangan besar</h2></div><a className="text-link" href="https://www.instagram.com/kamerain09/" target="_blank">Lihat @kamerain09 â</a></div><p className="gallery-note">Geser atau gunakan tombol panah untuk melihat foto dan video berikutnya.</p><div className="media-slider" onMouseEnter={()=>setGalleryPaused(true)} onMouseLeave={()=>setGalleryPaused(false)} onFocusCapture={()=>setGalleryPaused(true)} onBlurCapture={()=>setGalleryPaused(false)} onTouchStart={(e)=>setTouchStart(e.touches[0].clientX)} onTouchEnd={(e)=>{if(touchStart===null)return;const distance=e.changedTouches[0].clientX-touchStart;if(Math.abs(distance)>45)setGalleryIndex((galleryIndex+(distance<0?1:-1)+gallery.length)%gallery.length);setTouchStart(null)}}><button className="slider-arrow prev" aria-label="Media sebelumnya" onClick={()=>setGalleryIndex((galleryIndex-1+gallery.length)%gallery.length)}>â</button><div className="slide-stage">{gallery.map((media,index)=><figure key={media.src} className={`media-slide ${index===galleryIndex?"active":""}`} aria-hidden={index!==galleryIndex}>{media.type==="video"?<video src={media.src} poster={media.poster} controls preload="metadata" playsInline/>:<img src={media.src} alt={media.alt}/>}<figcaption><span>{String(index+1).padStart(2,"0")} / {String(gallery.length).padStart(2,"0")}</span><b>{media.caption}</b></figcaption></figure>)}</div><button className="slider-arrow next" aria-label="Media berikutnya" onClick={()=>setGalleryIndex((galleryIndex+1)%gallery.length)}>â</button></div><div className="slider-dots" role="tablist" aria-label="Pilih media">{gallery.map((media,index)=><button key={media.src} aria-label={`Tampilkan media ${index+1}`} aria-selected={index===galleryIndex} className={index===galleryIndex?"active":""} onClick={()=>setGalleryIndex(index)}>{media.type==="video"?"â¶":""}</button>)}</div><div className="slider-progress" aria-hidden="true"><span key={galleryIndex}></span></div></div></section>
    ...
    <section className="area wrap" data-reveal><div><span className="eyebrow">AREA LAYANAN</span><h2>Siap menemani momenmu ð</h2><p>Jakarta Timur, Jakarta Pusat, Tangerang dan area sekitarnya. Lokasi lain? Tanyakan saja melalui WhatsApp.</p></div><a className="btn light" href="#booking">Cek area & ketersediaan</a></section>
    <footer><div className="wrap footer"><div><a className="footer-logo" href="#top"><img src={`${import.meta.env.BASE_URL}kamerain-logo-v2.webp`} alt="Logo KameraIn"/></a><p>Rent it. Shoot it. Keep the memory.</p></div><div><b>Hubungi kami</b><a href="https://wa.me/6283193266639">WhatsApp Â· 0831-9326-6639</a><a href="https://www.instagram.com/kamerain09/">Instagram Â· @kamerain09</a></div><div><b>Jam layanan</b><p>Setiap hari<br/>08.00 â 21.00 WIB</p></div></div><div className="wrap copyright">Â© 2026 KameraIn09 â¢ Made with joy & instant memories.</div></footer>
    {bookingOpen&&<div className="booking-modal" role="dialog" aria-modal="true" aria-labelledby="booking-title" onMouseDown={(event)=>{if(event.target===event.currentTarget)setBookingOpen(false)}}><div className="booking-dialog"><button type="button" className="modal-close" aria-label="Tutup formulir" onClick={()=>setBookingOpen(false)}>Ã</button><span className="eyebrow coral">BOOKING KAMERAIN</span><h2 id="booking-title">Lengkapi data penyewaan</h2><p className="modal-intro">Data akan dicatat dengan status <b>Menunggu</b>. Setelah itu kamu langsung diarahkan ke WhatsApp KameraIn.</p><div className="booking-summary"><img src={cameras[camera].image} alt=""/><div><b>{cameras[camera].name}</b><small>{duration} â¢ {selectedPackage.name}</small></div><strong>{rupiah(total)}</strong></div><form onSubmit={submitBooking}><label>Nama lengkap<input name="nama" required minLength={3} autoComplete="name" placeholder="Contoh: Anggito Karta"/></label><label>Nomor WhatsApp<input name="whatsapp" required inputMode="tel" autoComplete="tel" pattern="[0-9+ -]{8,18}" placeholder="08xxxxxxxxxx"/></label><div className="form-row"><label>Tanggal penggunaan<input name="tanggalPenggunaan" type="date" min={today} required/></label><label>Lokasi/COD<input name="lokasi" required minLength={4} placeholder="Contoh: Jakarta Timur"/></label></div><label className="honeypot" aria-hidden="true">Perusahaan<input name="company" tabIndex={-1} autoComplete="off"/></label>{bookingState==="error"&&<p className="form-error">Data belum berhasil dikirim. Periksa koneksi lalu coba kembali.</p>}<button className="btn wide" disabled={bookingState==="sending"}>{bookingState==="sending"?"Mencatat bookingâ¦":"Catat booking & buka WhatsApp â"}</button><small className="privacy-note">Dengan melanjutkan, kamu menyetujui data digunakan untuk pengecekan jadwal penyewaan.</small></form></div></div>}
  </main>;
}
